...
from telegram import InlineKeyboardButton, InlineKeyboardMarkup, KeyboardButton, ReplyKeyboardMarkup, \
    ReplyKeyboardRemove, Update, ForceReply
from telegram.ext import Updater, CommandHandler, CallbackQueryHandler, ConversationHandler, MessageHandler, Filters, \
    CallbackContext

# Enable logging
from secret import TOKEN, ADMINS_IDS, ADMINS_GROUPCHAT
...
MAIN_MENU, ACCEPT_RULES, SUBMIT_PHOTO, SUBMIT_TITLE, SUBMIT_LINK, NSFW, IS_NSFW, ENTER_AT, ENTER_AUTHOR, CONFIRMATION = range(
    10)

userids_file = "./users"
submissions_file = "./submissions_{}.csv".format(uuid.uuid4())
fieldnames = ["filename",
              "title",
              "link",
              "nsfw",
...
def start(update: Update, context: CallbackContext) -> int:
    ...
    message += "👉 You can use /cancel to cancel what you are doing at any time, and /start to start over.\n" \
...
def accept_rules(update: Update, context: CallbackContext) -> int:
    ...
    update.message.reply_text("- Submissions will be open from February, 1st to February, 28th.\n"
    ...
    update.message.reply_text("Tl;DR if you want to partipate you need to send:\n"
    ...
    update.message.reply_text("Do you accept these rules?",
...
def submit_photo(update: Update, context: CallbackContext) -> int:
    ...
    update.message.reply_text("Perfect! Thank you very much!\n"
...
def tag_nsfw(update: Update, context: CallbackContext) -> int:
    ...
    update.message.reply_text("You're getting there!\n"
                              ...
                              reply_markup=ReplyKeyboardMarkup([["My artwork is NSFW", "My artwork is safe"]],
...
def is_nsfw(update: Update, context: CallbackContext) -> int:
    update.message.reply_text("Please explicitly list everything that is NSFW in your artwork.\n",
                              reply_markup=ReplyKeyboardRemove())
...
def store_nsfw(update: Update, context: CallbackContext) -> int:
    ...
    update.message.reply_text("Every artwork needs credit!\n"
...
def enter_author(update: Update, context: CallbackContext) -> int:
    ...
    update.message.reply_text("We are almost done!\n"
...
def confirmation(update: Update, context: CallbackContext) -> int:
    ...
    update.message.reply_text("Are these information correct?\n",
                              reply_markup=ReplyKeyboardMarkup([["Yes!", "Nah."]], one_time_keyboard=True))
...
def cancel(update: Update, context: CallbackContext) -> int:
    update.message.reply_text(
        "Come back anytime with /start!", reply_markup=ReplyKeyboardRemove()
    )
        ...
        with open(userids_file, "r") as f:
            ...
            if update.message.reply_to_message:
                for userid in userids:
                    try:
                        context.bot.send_message(userid, update.message.reply_to_message.text)
                    except:
                        blocked += 1
                update.message.reply_text(
...
def main() -> None:
    ...
    conv_handler = ConversationHandler(
        ...
        states={
            MAIN_MENU: [MessageHandler(Filters.regex('^Submit$'), accept_rules),
                        MessageHandler(Filters.regex('^(\(admin\) )?Vote$'), cancel)],
            ACCEPT_RULES: [MessageHandler(Filters.regex('^Yes!$'), submit_photo),
            ...
            NSFW: [MessageHandler(Filters.regex('^My\ artwork\ is\ NSFW$'), is_nsfw),
            ...
            CONFIRMATION: [MessageHandler(Filters.regex('^Yes!$'), submission),
